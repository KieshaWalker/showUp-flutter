...
/// Main habits tracking screen displayed in the AppShell.
///
/// Widget hierarchy:
/// HabitsScreen (ConsumerWidget)
///   ↓
/// Scaffold (main container)
///   ├─ AppBar (title + calendar action)
///   ├─ body: ListView containing:
///   │   ├─ _WeekStrip (horizontal week progress view)
///   │   ├─ "Today" section with _HabitTile widgets
///   │   └─ "Completed" section with _HabitTile widgets
///   └─ FloatingActionButton (add new habit)
///
/// Data flow:
/// 1. Watches habitsNotifierProvider (provided by HabitsNotifier)
/// 2. Receives AsyncValue<List<HabitWithStatus>>
/// 3. Splits habits into active (not done) and done (completed this week)
/// 4. Renders each as a _HabitTile with completion toggle
/// 5. Long press on tile triggers habit deletion dialog
...
/// Displays a visual calendar strip showing the current week (Mon-Sun).
/// Each day shows a circle with today highlighted in terracotta, past days in gray.
///
/// Role in widget tree:
/// - Rendered at the top of HabitsScreen's body in the ListView
/// - Provides weekly progress context without requiring interaction
/// - Acts as a visual calendar reference for habit tracking
///
/// Data requirements:
/// - Receives List<HabitWithStatus> habits (not directly used for rendering,
///   could be used for future week-level analytics)
/// - Calculates week bounds automatically from current date
...
/// Individual habit card widget displaying habit details and controls.
///
/// Widget hierarchy:
/// _HabitTile (ConsumerWidget)
///   ↓
/// Material + InkWell (desktop-friendly click detection)
///   ├─ onTap: Handle habit tap (placeholder)
///   ├─ onLongPress: Delete habit
///   ↓
/// Container (card styling)
///   ↓
/// Row (horizontal layout)
///   ├─ Left: GestureDetector → completion circle icon
///   ├─ Center: Expanded Column with habit name + frequency text
///   └─ Right: Streak badge (🔥 + count)
///
/// Interactive elements:
/// - Completion circle (tap) → toggles isDone, calls toggleCompletion() notifier
/// - Entire tile (tap) → placeholder for future detail view
/// - Entire tile (long press) → confirms and deletes habit
///
/// Data flow:
/// - Receives HabitWithStatus (computed data from notifier)
/// - isDone determines visual opacity (faded if complete)
/// - Displays streak, frequency type, and name
/// - User interactions trigger notifier methods (toggleCompletion, delete)
          ...
          child: InkWell(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color:
                    isDone
                        ? AppColors.eucalyptus.withOpacity(0.08)
